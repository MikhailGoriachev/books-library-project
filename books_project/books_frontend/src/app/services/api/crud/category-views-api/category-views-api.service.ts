...
export class CategoryViewsApiService {
    ...
    create(item: CategoryView) {
        const dto = new CategoryViewDto(0, item.user.id, item.categoryName.id, item.viewedAt);
        return this._apiService.post(CategoryViewsApiService.basePath, item);
    }

    update(item: CategoryView) {
        const dto = new CategoryViewDto(item.id, item.user.id, item.categoryName.id, item.viewedAt);
